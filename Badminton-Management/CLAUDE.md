...
- **`pwa.js`** — install-to-home-screen + pull-to-refresh rules (`PWA` global). Decides *whether* to offer an install (`installAvailability`, `shouldShowBanner`, `shouldShowIcon`, `isStandaloneDisplay`, `isIOS`/`isIOSSafari`) and does the pull gesture math (`createPullTracker`, `pullDistance`). Everything the browser knows is passed in as arguments, so it stays DOM-free and testable. See `documents/pwa-install-and-pull-to-refresh.md`.
- **`sw.js` / `manifest.webmanifest` / `icons/`** — the installable-app (PWA) layer. The service worker exists for installability plus an offline fallback of the *shell only*: same-origin files are network-first, the three CDN hosts are stale-while-revalidate, and every Firebase/Google data host is **never** cached so live data stays live. Icons are regenerated by `node tools/make-icons.js` (pure Node `zlib`, no dependency) — edit that script, not the PNGs. Bump `VERSION` in `sw.js` when shell files change meaningfully.
...
The app installs to the home screen and supports pull-to-refresh:

- **Install banner** (`#install-banner`, bottom of the screen) appears only when an install is really
  possible — Chrome/Edge hand us a `beforeinstallprompt` event, or we're on iOS *Safari* (no such
  event exists there, so the banner shows the manual share-sheet steps instead).
- **Dismissing hides the banner, never the ability to install.** The close button snoozes the banner
  for `PWA.SNOOZE_MS` (7 days, key `bmInstallDismissedAt`) and the header install icon
  (`#install-icon-btn`) takes over. Both disappear for good once `appinstalled` fires or the page is
  running standalone. Keep that invariant — it's covered by a regression test in `pwa.test.js`.
- **Pull-to-refresh**: `PWA.createPullTracker` drives `#ptr-indicator` and reloads past a 70px pull.
  Only fires at document scroll top, with no scrolled ancestor under the finger, and never while the
  live scorer / QR modal / sync overlay is open. Native browser pull-to-refresh is switched off via
